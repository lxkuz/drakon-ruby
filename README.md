...
Для **ациклических** схем получается класс в стиле сервис-объекта: **`DoThing.call(ctx)`** или **`DoThing.call(flag: true)`** (без `ctx` задаётся `OpenStruct` из kwargs). Внутри: `def self.call(ctx = nil, **kwargs)`, экземплярный **`call(ctx)`**, алиасы **`run`** и **`start`**. Если в графе есть **цикл**, используется машина состояний по id узлов — тот же внешний `.call`.
